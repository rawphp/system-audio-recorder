...
- [x] `docs/user-guide.md` exists and is valid GitHub-flavored Markdown.
- [x] All ten section headings listed in the Task section are present, in the listed order.
- [x] Each `x-apple.systempreferences:` URL in the guide matches an existing constant in `App/Errors/PermissionDeepLink.swift` (or, for Accessibility, a documented Apple-canonical URL added to `PermissionDeepLink.swift` if not already present).
- [x] Image references resolve: every `![...](user-guide-assets/...)` path points to a file produced by REQ-053 or REQ-054.
- [x] The output-folder section does NOT state `~/Music/Recordings/` as a fixed location — it qualifies it as "the default" and points to Settings + Reveal in Finder.
- [x] Document length is between 600 and 2000 words (excluding images).
- [x] No developer-facing instructions (no `make`, no `xcodegen`, no `xcodebuild`, no terminal commands) appear in the guide.
- [x] The guide renders cleanly on github.com — verified by previewing the file in a Markdown renderer or pushing to a branch and checking the GitHub blob view.
...
- `docs/user-guide.md` — end-user guide, 1030 words, 16 headings, 9 embedded images, all acceptance criteria met.
