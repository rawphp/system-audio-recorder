...
- Tests placed in `Tests/AudioEngineTests/` (existing target) rather than a new `PermissionsTests` target — adding a new XcodeGen target was non-trivial and the existing target already compiles all engine + supporting code via `@testable import SystemAudioRecorder`. Documented here per instructions.
