...
- `App/SystemAudioRecorderApp.swift` — Added `@State private var dockPolicyController: DockPolicyController?`; wired in `.onAppear` alongside `MenuBarController` — creates controller from `appStore.settings` and calls `controller.start()`.
