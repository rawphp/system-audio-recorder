...
- [ ] Sidecar `.recording.json` exists and updates every 1 s during a session
- [ ] On clean stop, sidecar is deleted (no recovery prompt next launch)
- [ ] On simulated crash (kill -9), next launch detects the sidecar and shows the recovery prompt with the session's date
- [ ] Recovery succeeds: WAV header `dataChunkSize` is repaired to match actual sample count on disk; the file opens via `AVAudioFile` without throwing; subsequent encoding via REQ-017 produces a playable MP3 of duration ≥ pre-crash recording length
- [ ] User dismissal options work as specified (encode / keep WAV / discard)
